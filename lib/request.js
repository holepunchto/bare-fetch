...
module.exports = class Request extends Body {
  ...
  constructor(input, init = {}) {
    ...
    const {
      ...
      redirect = input.redirect === undefined ? 'follow' : input.redirect,
      credentials = input.credentials === undefined ? 'same-origin' : input.credentials,
    ...
    if (redirect !== 'follow' && redirect !== 'error' && redirect !== 'manual') {
      throw errors.INVALID_REDIRECT(`Invalid redirect mode '${redirect}'`)
    }

    this._redirect = redirect

    if (credentials !== 'omit' && credentials !== 'same-origin' && credentials !== 'include') {
      throw errors.INVALID_CREDENTIALS(`Invalid credentials mode '${credentials}'`)
    }
